...
const faders = document.querySelectorAll(".listSquare");
var i = 0;
  ...
  i = 0;
    entries.forEach(entry => {
        if(!entry.isIntersecting){
          ...
          for(let index = 0; index < entry.target.childElementCount; index++){
            setTimeout(
              () => {
                entry.target.children[index].classList.add("appear");
              }, index*200);
          }
